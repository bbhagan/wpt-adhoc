...
const WATCH_TEST_CHECK_FREQUENCY = Number(
  process.env.WATCH_TEST_CHECK_FREQUENCY
);
...
class Index extends React.Component {
  ...
  static async getInitialProps({ query }) {
    ...
    initialProps.testLocations = await fetchLocations(
    ...
    try {
      const tests = await APISubmitTests(
        //filter the tests for populated URLs and active locations
        {
          ...
          testLocations: this.state.testLocations.filter(
  ...
  watchTest = (testToWatch, afterTest) => {
    ...
    const checkMaxWatcTestsInterval = setInterval(async () => {
      if (this.activeWatchRequests < WATCH_TEST_MAX_REQUESTS) {
        ...
        newTest = await fetchTestResults(
          testToWatch,
  ...
  render() {
    ...
    if (this.state.afterTests) {
      afterTestsInProgress = this.state.afterTests.filter(
        (test) => test.completed === false
      );
      afterTestsCompleted = this.state.afterTests.filter(
          ...
          <div className="container">
            <TestConfiguration
              ...
              testLocationFetchError={
                this.props.testLocations.testLocationFetchError
              }
            />
