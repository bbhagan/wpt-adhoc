...
router.post('/', (req, res) => {
    ...
    req.body.testUrls.forEach(testObj => {
        if(testObj.url && testObj.url !== "" && Number.isInteger(testObj.index)) {
            ...
            req.body.testLocations.forEach(location => {
                ...
                promises.push(
                    ...
                    .then(resJson => {
                        return({url: testObj.url, location: location.location, testId: resJson.data.testId});
                    })
                    .catch(error => {
                        console.log(`Promise rejected: ${error}`);
                        return res.json({statusCode: 400, statusMsg: `submitTests error: ${error}`});
                    })
                );
            });
        ...
        if (!atLeastOneTest) {
            return res.json({statusCode: 400, statusMsg: `submitTests error: No valid tests`});
        }
