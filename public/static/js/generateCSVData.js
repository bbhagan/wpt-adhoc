    ...
    tests.forEach((test) => {
      ...
      if (afterTests.length) {
        afterTests.forEach((afterTest) => {
          if (
            test.data.url === afterTest.data.url &&
            test.data.location === afterTest.data.location
          ) {
      ...
      //Description header
      csvData.push([
        getTestTextHeader(test, matchingAfterTest ? " (Before)" : ""),
      ]);
      ...
      if (Object.entries(matchingAfterTest).length !== 0) {
        ...
        //Average Line
        csvData.push(
          getAvgRow(matchingAfterTest, testConfig.resultOptions, mobDesk)
        );

        //Difference
        csvData.push(
          getDifferenceRow(test, matchingAfterTest, testConfig.resultOptions)
        );
        //Winner
        csvData.push(
          getWinnerRow(
  ...
  try {
    ...
    testSets.forEach((testSet) => {
      testConfig.resultOptions.forEach((resultOption) => {
        const testsSortedByField = sortTestsByField(
          testSet,
          resultOption.wptField
        );
